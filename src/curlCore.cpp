...
string getRemoteData(std::string &url, bool admin) //quiza es string url
{
	...
	if (switcher->curl && f_curl_setopt && f_curl_perform) {
		...
		if (admin) {
			f_curl_setopt(switcher->curl, CURLOPT_HTTPAUTH,
				      (long)CURLAUTH_BASIC);
...
void setCurls()
{
	...
	switcher->curlJudgements =
		switcher->curlContest + "/judgements?submission_id=";
...
void getTeamsContestInfo(contestInfo &contest, bool startingInfo)
{
	...
	string jsonScoreboardInfo =
		getRemoteData(switcher->curlScoreboard, false);
	obs_data_t *dataScoreboardInfo =
		obs_data_create_from_json(jsonScoreboardInfo.c_str());
	obs_data_array_t *arrayScoreboard =
		obs_data_get_array(dataScoreboardInfo, "rows");
	...
	for (size_t i = 0; i < sizeScoreboard; i++) {
		...
		obs_data_array_t *problemsTeam =
			obs_data_get_array(team, "problems");
		...
		if (startingInfo) {
			...
			contest.scoreBoard.insert(
				pair<string, teamInfo>(id, info));
		} else {
			double temp = (switcher->rankWeight / info.rank) +
				      (switcher->numPendingWeight *
				       (info.score.num_pending / info.rank)) -
				      (switcher->timeInStreamWeight *
				       (info.timeInStream * info.rank));
...
void getJudgementsInfo(contestInfo &contest)
{
	...
	map<string, submissionInfo>::iterator it =
		contest.submissionPendings.begin();
	int i = 0;
	...
	while (it != contest.submissionPendings.end() &&
		...
		string jsonFixedJudgementsInfo = vectorToObj(
			jsonJudgementsInfo); //Como el servidor devuelve el vector directamente, es necesario una adaptación para usar la función del obs
		obs_data_t *dataJudgementsInfo = obs_data_create_from_json(
			jsonFixedJudgementsInfo.c_str());
		obs_data_array_t *arrayJudgements =
			obs_data_get_array(dataJudgementsInfo, "array");

		obs_data_t *judgements =
			obs_data_array_item(arrayJudgements, 0);
		it->second.result =
			obs_data_get_string(judgements, "judgement_type_id");
...
void getIdentificationTeamInfo(string id, teamInfo &info)
{
	...
	obs_data_t *dataTeamInfo =
		obs_data_create_from_json(jsonFixedTeamInfo.c_str());
	obs_data_array_t *arrayTeamInfo =
		obs_data_get_array(dataTeamInfo, "array");

	obs_data_t *tamInfo = obs_data_array_item(arrayTeamInfo, 0);

	info.identificationInfo.affiliation =
		obs_data_get_string(tamInfo, "affiliation");
	info.identificationInfo.name = obs_data_get_string(tamInfo, "name");
	info.identificationInfo.nationality =
		obs_data_get_string(tamInfo, "nationality");
	       ...
	       contestInfo &contest)
{
	...
	if (bestHeuristic >=
		...
		if (switcher->lastTeamsInStream.size() == 0 ||
		    bestId != switcher->lastTeamsInStream.back()) {
			//Busqueda del elemento en la base de datos estática
			map<string, IpsTeam>::iterator it1 =
				switcher->ipsContestData.ipsTeams.find(
					best.identificationInfo
						.name); //IpsContest y la información obtenida por curl se conectan gracias a que el equipo tiene el MISMO NOMBRE
			...
			switcher->textTeamContent =
				"Team: " +
				best.identificationInfo.name.substr(0, 31) +
				"\nClassification: " +
				std::to_string(best.rank) +
				"\nNumber of Problem Solved: " +
				std::to_string(best.score.num_solved);

			/*Actualización de las variables de control*/
			switcher->swapIp = true;
			if (obs_scene_from_source(
				    obs_frontend_get_current_scene()) ==
			    switcher->classificationScene) // switcher->lastTeamsInStream.size() > 0 && switcher->lastTeamsInStream.back() == switcher->tokenIdClassification
	...
	} else //La clasificación tiene mejor puntuación que la escena mejor valorada.
	{
		...
		if (obs_scene_from_source(obs_frontend_get_current_scene()) ==
		    switcher->teamViewerScene)
			switcher->swapScene = true;

		switcher->lastTeamsInStream.push(
			switcher->tokenIdClassification);
		switcher->classificationTimeInStream++;
	}

	//Actualización de las variables para la heurística
	if (switcher->lastTeamsInStream.size() ==
	    switcher->cycleSize +
		    1) //El mas uno es para tener en cuenta que durante esta función se inserta un nuevo elemento
	{
